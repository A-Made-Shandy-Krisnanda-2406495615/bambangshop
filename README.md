...
1. Pada implementasi ini, pola yang digunakan adalah push model. Publisher secara aktif mengirimkan data notifikasi kepada setiap subscriber ketika terjadi event tertentu pada product, seperti `CREATED`, `DELETED`, atau `PROMOTION`. Hal ini terlihat dari `NotificationService::notify`, yang membentuk payload notifikasi dan langsung mengirimkannya ke seluruh subscriber yang terdaftar tanpa menunggu subscriber meminta data terlebih dahulu.

2. Jika pendekatan yang digunakan adalah pull model, maka subscriber tidak akan menerima data notifikasi secara langsung dari publisher, melainkan hanya menerima penanda bahwa telah terjadi perubahan dan kemudian mengambil data yang dibutuhkan secara mandiri. Kelebihan pendekatan ini adalah subscriber memiliki fleksibilitas lebih tinggi untuk menentukan kapan dan data apa yang ingin diambil. Namun, kekurangannya adalah beban logika pada sisi subscriber menjadi lebih besar dan proses sinkronisasi dapat menjadi lebih lambat karena setiap subscriber harus melakukan request tambahan ke publisher.

3. Multithreading diperlukan agar proses pengiriman notifikasi ke banyak subscriber dapat berjalan secara paralel. Jika seluruh pengiriman dilakukan secara sequential, maka keterlambatan pada satu subscriber dapat memperlambat pengiriman ke subscriber lainnya. Dalam konteks aplikasi web, kondisi tersebut akan menurunkan responsivitas sistem. Dengan menggunakan thread terpisah untuk setiap subscriber, publisher dapat mengirim notifikasi secara lebih efisien dan mengurangi bottleneck pada proses update.
